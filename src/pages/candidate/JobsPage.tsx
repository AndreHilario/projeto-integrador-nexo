...
export function JobsPage() {
  ...
  const jobs = useMemo(() => database.jobs.filter((job) => {
    const term = search.toLowerCase()
    return job.status === 'active'
      && (!term || `${job.title} ${job.companyName} ${job.skills.join(' ')}`.toLowerCase().includes(term))
      && (workplace === 'Todas' || job.workplace === workplace)
      && (experience === 'Todos' || job.experience === experience)
  }), [database.jobs, experience, search, workplace])
  ...
  return (
    <div className="page-stack">
      ...
      <section className="search-panel glass">
        <label className="main-search"><Search size={21} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Cargo, empresa ou habilidade" /></label>
      ...
      <section className="content-section">
        <div className="list-toolbar">
          <div><h2>Vagas recomendadas</h2><p>{jobs.length} oportunidades com o seu perfil</p></div>
