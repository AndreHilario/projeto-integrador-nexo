...
export function CompanyDashboardPage() {
  ...
  const jobs = database.jobs.filter((job) => job.companyId === currentUser?.id)
  ...
  return (
    <div className="page-stack">
      ...
      <section className="content-section">
        ...
        {jobs.length > 0 ? <div className="jobs-grid">{jobs.map((job) => <JobCard key={job.id} job={job} companyView />)}</div> : <div className="empty-state surface"><BriefcaseBusiness size={32} /><h3>Publique sua primeira vaga</h3><p>Leva poucos minutos para começar a receber candidatos.</p><Link className="primary-button" to="/empresa/vagas/nova"><Plus size={17} /> Nova vaga</Link></div>}
      </section>
    </div>
  )
}
